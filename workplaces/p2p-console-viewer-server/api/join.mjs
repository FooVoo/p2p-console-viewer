import { isValidRoomName } from '../lib/validation.mjs';
import {
	MAX_CLIENTS,
	MAX_ROOM_CLIENTS,
	MESSAGE_BURST,
...
import { KvRoomManager } from '../lib/redis-client.mjs';
...
export function createJoinHandler(roomManager, allowedOrigins = ALLOWED_ORIGINS) {
	return withCors(async (request) => {
		try {
			...
			return new Response(JSON.stringify({ id, room: result.room, peers: result.peers }), {
				status: 200
			});
		} catch (err) {
...
const _joinHandler = createJoinHandler(new KvRoomManager());
