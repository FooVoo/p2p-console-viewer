...
export class RestSignalingClient {
  ...
  async _poll() {
    ...
    try {
      ...
      this.emitError(
        new Error(`Poll failed: ${error.message || "Unknown error"}`),
      );
    }
  }
      ...
      case "ice-candidate": {
        ...
        if (p2p) {
          p2p.addIceCandidate(data.candidate).catch((error) => {
            console.error(
              `Failed to add ICE candidate from ${from}:`,
              error,
            );
          });
        } else {
  ...
  createP2PConnection(remotePeerId) {
    ...
    p2p.onOffer((offer) => {
      this._sendSignal({
        ...
        this.emitPeerError(
          remotePeerId,
          new Error(`Failed to send offer: ${error.message}`),
        );
      });
    });
    ...
    p2p.onAnswer((answer) => {
      this._sendSignal({
        ...
        this.emitPeerError(
          remotePeerId,
          new Error(`Failed to send answer: ${error.message}`),
        );
      });
    });
    ...
    p2p.onMessage((message) => {
      this.onPeerMessageHandlers.forEach((h) => {
        try {
          ...
          console.error(
            `Error in peer message handler for ${remotePeerId}:`,
            e,
          );
        }
      });
  ...
  async initiateP2P(remotePeerId) {
    ...
    try {
      ...
      console.error(
        `Failed to initiate P2P connection with ${remotePeerId}:`,
        error,
      );
